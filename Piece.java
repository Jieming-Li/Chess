...
public class Piece {
    ...
    public boolean isMoveLegal(Board board, int endRow, int endCol) {
        switch (this.character) {
            ...
            case '\u265f':
                ...
                //if the move is legal and the destination is the end row, then call pawnPromotion method
                if ((endRow==0 || endRow==7) && (pawn.isMoveLegal(board, endRow, endCol))) {
                    this.pawnPromotion();
                }
    ...
    //set the character attribute of the piece
    public void setCharacter(char character){
        this.character = character;
    }
    // to promote the pawn to the character indicated by the user
    public void pawnPromotion() {
        System.out.println("What do you want to promote the pawn to? (Choose from 'queen','rook','bishop','knight')");
        Scanner myScanner = new Scanner(System.in);
        String input = myScanner.nextLine();
        //check user input and set the character
        if (this.getIsBlack()) {
            if (input.equals("queen")) {
                ...
                this.pawnPromotion(); //if the input is invalid, calls the method again
            }
        } else {
            if (input.equals("queen")) {
                this.setCharacter('\u2655');
